...
export class ManualResultService {
  ...
  totalSaleOnCurrentDraw(game_type_id, combination_id){
    return this.http.post<ServerResponse>(this.BASE_API_URL +'/totalSaleOnCurrentDraw', {game_type_id, combination_id})
    .pipe(catchError(this.errorService.serverError), tap(response => {
      // console.log(response);
    }))
  }
