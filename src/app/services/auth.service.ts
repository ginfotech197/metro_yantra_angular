...
export class AuthService {
    ...
    return this.http.post<AuthResponseData>(this.BASE_API_URL + '/login', loginData)
      .pipe(catchError(this.serverError), tap(resData => {
        ...
        if (resData.success === 1){
          ...
          // if (resData.data.user.blocked === 1){
  ...
  logout(){
    // this.http.get(this.BASE_API_URL + '/logout/' + this.user.userId).subscribe((response: ServerResponse) => {
    //   const userData: User = JSON.parse(localStorage.getItem('user'));
    ...
    this.http.get(this.BASE_API_URL + '/logout').subscribe((response: ServerResponse) => {
      this.userBehaviorSubject.next(null);
      localStorage.removeItem('user');
      this.router.navigate(['/power']).then(r => {
          if (r) {
