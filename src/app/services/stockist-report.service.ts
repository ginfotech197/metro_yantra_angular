...
export class StockistReportService {
  ...
  turnOverReport(start_date, end_date, stockist_id) {
    return this.http.post<{ success: number; data: any }>(this.BASE_API_URL + '/stockist/turnOverReport', { start_date, end_date, stockist_id })
      .pipe(catchError(this.handleError), tap(((response: ServerResponse) => {
        if (response.data) {
