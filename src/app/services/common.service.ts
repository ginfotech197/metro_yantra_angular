...
export class CommonService {
  ...
  getLiveDrawTimeSubject = new Subject<any>();
  user: User;
  ...
  constructor(private http: HttpClient, private errorService: ErrorService) {
    ...
    this.http.get(this.BASE_API_URL + '/serverTime').subscribe((response: {hour: number, minute: number, second: number, 'meridiem': string}) => {
  ...
  resetPassword(oldPassword, newPassword){
    ...
    return this.http.patch<any>(this.BASE_API_URL + '/resetPassword', {userId: this.user.userId, password: oldPassword, newPassword : newPassword})
  ...
  getLiveDraw(){
    this.http.get(this.BASE_API_URL + '/getLiveDraw').subscribe((response: ServerResponse) => {
      this.getLiveDrawTime = response.data;
      this.getLiveDrawTimeSubject.next({...this.getLiveDrawTime});
    });
  }
  ...
  checkPinValidation(email){
    return this.http.post(this.BASE_API_URL + '/pinCheckValidation', {email: email})
      .pipe(catchError(this.errorService.serverError), tap(response => {
      }));
  }
